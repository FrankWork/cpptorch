...
class MultiHeadedAttention(nn.Module):
	...
	def forward(self, query, key, value, mask=None):
		...
		x, self.attn = attention(query, keuy, value, mask=mask,
...
for epoch in range(10):
	...
	run_epoch(data_gen(V, 30, 20), model, loss_fn)
	model.eval() # set in eval mode
	loss_fn = SimpleLossCompute(model.generator, criterion, None)
	print(run_epoch(data_gen(V, 30, 5), model, loss_fn)
